...
const TodayPage = () => {
    ...
    const fetchTodos = async () => {
        try {
            ...
            const overdueTodos = response.data.filter(todo => {
                const todoDateTime = new Date(todo.dueDateTimestamp);
                return todoDateTime < currentDateTime;
            });

            const todayTodos = response.data.filter(todo => {
                const todoDateTime = new Date(todo.dueDateTimestamp);
                return todoDateTime >= currentDateTime && 
                       todoDateTime >= todayStart && 
                       todoDateTime <= todayEnd;
            });
    ...
    return (
        <Box sx={{ display: 'flex' }}>
            ...
            >
                ...
                <IconButton onClick={() => navigate('/upcoming')}>
                    <CalendarTodayIcon />
                </IconButton>
                {currentUser?.avatar && (
                    <Avatar 
